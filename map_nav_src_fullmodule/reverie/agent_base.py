...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            path_segments = v['path']  # 每个元素是一个路径段（节点列表）
            view_segments = v.get('view', [[]] * len(path_segments))  # 每个路径段的视角列表
            ...
            for step_path, step_views in zip(path_segments, view_segments):
                # 统一处理路径格式
                step_path = [step_path] if not isinstance(step_path, list) else step_path

                # 统一处理视角格式
                if not isinstance(step_views, list):
                    step_views = [step_views] if step_views else [{'heading': None, 'elevation': None}]

                # 确保视图与路径节点对应
                if len(step_views) != len(step_path):
                    step_views = [{'heading': None, 'elevation': None}] * len(step_path)

                # 收集完整路径信息
                for vp, view in zip(step_path, step_views):
                    full_path.append(vp)
                    full_views.append({
                        'viewpoint': vp,
                ...
                if step_path:
                    ...
                    step_summaries.append([
                        step_path[-1],
                        last_view.get('heading'),
                        last_view.get('elevation')
                    ])

            item = {
                'instr_id': k,
                'trajectory': path_segments,  # 原始路径段结构
                'path_segments': step_summaries,  # 每步的最终位置+朝向
                'full_trajectory': full_path,  # 所有节点的顺序列表
                'full_views': full_views,  # 所有节点的完整视角信息
