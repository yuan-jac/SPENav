...
from map_nav_src_fullmodule.models.model import VLNBert, Critic
from map_nav_src_fullmodule.models.ops import pad_tensors_wgrad
from torch.nn.utils.rnn import pad_sequence
from map_nav_src_fullmodule.utils.distributed import is_default_gpu
from map_nav_src_fullmodule.utils.ops import pad_tensors, gen_seq_masks
...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_view_img_fts,batch_view_text_fts, batch_obj_img_fts, batch_loc_fts, batch_nav_types = [], [], [], [],[]
        batch_view_lens, batch_obj_lens = [], []
        batch_cand_vpids, batch_objids = [], []

        for i, ob in enumerate(obs):
            view_text_fts,view_img_fts, view_ang_fts, nav_types, cand_vpids = [],[], [], [], []
            # cand views
            used_viewidxs = set()
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        return {
            ...
            'grid_spatial_text': [
                torch.stack(
                    [torch.stack([
                        torch.tensor(view) if not isinstance(view, torch.Tensor) else view
                        for view in nav_point  # 每个导航点的12个方向
                    ], dim=0) for nav_point in obs[index]['spatial_fts']],  # 多个导航点
                    dim=0  # 拼接为 [n, 12, 768]
                ).cuda()
    ...
    def make_equiv_action(self, a_t, gmaps, obs, traj=None):
        """
        全景动作到第一人称动作转换器
        保存路径中所有节点的heading和elevation信息
        """
        for i, ob in enumerate(obs):
            action = a_t[i]
            current_scan = ob['scan']
            current_vp = ob['viewpoint']

            if action is not None:  # 非停止动作
                # 获取路径（不包含起点）
                path = gmaps[i].graph.path(current_vp, action)
                traj[i]['path'].append(path)

                # 为路径中的每个节点计算视角信息
                segment_views = []
                prev_vp = current_vp  # 从当前视角开始

                for next_vp in path:
                    # 计算从prev_vp到next_vp的视角
                    view_key = f"{current_scan}_{prev_vp}"
                    viewidx = self.scanvp_cands[view_key][next_vp]
                    ...
                    prev_vp = next_vp  # 更新前一个节点

                # 保存这段路径的所有视角信息
                traj[i]['view'].append(segment_views)

                # 对模拟器只需要最后一个动作的视角
                if segment_views:
                    last_view = segment_views[-1]
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        traj = [{
            ...
            'path': [[ob['viewpoint']]],
            'view': [[{  # 改为二维列表，保持与后续 append 保持一致
                'viewpoint': ob['viewpoint'],
                'heading': ob.get('heading', None),
                'elevation': ob.get('elevation', None)
            }]],
            'pred_objid': None,
            'details': {},
        } for ob in obs]
            ...
            for i in range(batch_size):
                if (not ended[i]) and just_ended[i]:
                    ...
                    if stop_node is not None and obs[i]['viewpoint'] != stop_node:
                        path = gmaps[i].graph.path(obs[i]['viewpoint'], stop_node)
                        traj[i]['path'].append(path)
                        segment_views = []
                        ...
                        for next_vp in path:
                            view_key = f"{scan}_{prev_vp}"
                            viewidx = self.scanvp_cands[view_key].get(next_vp, None)
                            if viewidx is not None:
                                heading = (viewidx % 12) * math.radians(30)
                                elevation = (viewidx // 12 - 1) * math.radians(30)
                            else:
                                heading = None
