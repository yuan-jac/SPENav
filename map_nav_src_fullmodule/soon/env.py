...
import numpy as np
import torch
from map_nav_src_fullmodule.reverie.env import EnvBatch
from shapely.geometry import Point
from shapely.geometry.polygon import Polygon
from map_nav_src_fullmodule.soon.data_utils import normalize_angle
from map_nav_src_fullmodule.utils.data import angle_feature, get_all_point_angle_feature
from map_nav_src_fullmodule.utils.data import load_nav_graphs, new_simulator
    ...
    def _get_gt_trajs(self, data):
        ...
        for path_id, value in gt_trajs.items():
            ...
            for vp, bbox in value['bboxes'].items():
                ...
                new_bbox['target']['left_top']['elevation'] = (new_bbox['target']['left_top'][
                                                                   'elevation'] + math.pi) / (2 * math.pi)
                new_bbox['target']['right_bottom']['heading'] = new_bbox['target']['right_bottom']['heading'] / (
                        2 * math.pi)
                new_bbox['target']['right_bottom']['elevation'] = (new_bbox['target']['right_bottom'][
                                                                       'elevation'] + math.pi) / (2 * math.pi)
                new_bbox['target']['left_bottom']['heading'] = new_bbox['target']['left_bottom']['heading'] / (
                        2 * math.pi)
                new_bbox['target']['left_bottom']['elevation'] = (new_bbox['target']['left_bottom'][
                                                                      'elevation'] + math.pi) / (2 * math.pi)
                new_bbox['target']['right_top']['heading'] = new_bbox['target']['right_top']['heading'] / (2 * math.pi)
                new_bbox['target']['right_top']['elevation'] = (new_bbox['target']['right_top'][
                                                                    'elevation'] + math.pi) / (2 * math.pi)
    ...
    def _eval_item(self, pred_path, obj_heading, obj_elevation, gt_item):
        ...
        if path[-1] in gt_bboxes:
            ...
            gt_poly = Polygon([(gt_bbox['target']['left_top']['heading'], gt_bbox['target']['left_top']['elevation']),
                               ...
                               (gt_bbox['target']['right_bottom']['heading'],
                                gt_bbox['target']['right_bottom']['elevation']),
                               (gt_bbox['target']['left_bottom']['heading'],
                                gt_bbox['target']['left_bottom']['elevation'])])
    ...
    def eval_metrics(self, preds):
        ...
        for item in preds:
            ...
            traj_scores = self._eval_item(traj,
                                          item['trajectory']['obj_heading'][0], item['trajectory']['obj_elevation'][0],
                                          gt_item)
