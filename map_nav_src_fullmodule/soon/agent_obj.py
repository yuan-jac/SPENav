...
import numpy as np
import torch
from map_nav_src_fullmodule.models.graph_utils import GraphMap
from map_nav_src_fullmodule.models.model import VLNBert, Critic
from map_nav_src_fullmodule.reverie.agent_obj import GMapObjectNavAgent
...
class SoonGMapObjectNavAgent(GMapObjectNavAgent):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            path_segments = v['path']  # 原始路径（每一步可能是中继路径）
            views = v.get('view', [None] * len(path_segments))  # 每一步的朝向

            # 构造 viewpoint + heading + elevation 三元组
            heading_segments = []
            for step_path, view in zip(path_segments, views):
                # 当前 step 的最后一个 viewpoint 表示当前位置
                vp = step_path[-1] if isinstance(step_path, list) else step_path
                heading = view.get('heading', None) if view else None
                elevation = view.get('elevation', None) if view else None
                heading_segments.append([vp, heading, elevation])

            item = {
                'instr_id': k,
                'trajectory': path_segments,  # 保留完整路径段（原始 viewpoint list）
                'path_segments': heading_segments,  # 每步当前位置 + 朝向信息
