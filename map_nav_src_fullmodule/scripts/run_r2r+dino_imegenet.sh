...
name=${name}-seed.${seed}
name=${name}-init.aug.45k-new_dino+siglip+nomrc+2+test-7.11
...
CUDA_VISIBLE_DEVICES='3' python3 -m torch.distributed.launch --master_port 29510 --nproc_per_node=${ngpus}  map_nav_src_fullmodule/main_nav.py $flag \
    --tokenizer bert \
    --bert_ckpt_file datasets/R2R/exprs_map/pretrain/cmt-vitbase-mlm.sap-init.lxmert-aug.speaker-new-nock-allmodule-dino200000-3/ckpts/model_step_190500.pt\
...
#CUDA_VISIBLE_DEVICES='0,1,2,3,4,5' python3 -m torch.distributed.launch --master_port 29502 --nproc_per_node=${ngpus}  main_nav.py $flag  \
	 ...
	 #--resume_file  datasets/R2R/exprs_map/finetune/Grid_Map-dagger-dino_imagenet-seed.0-init.aug.45k-new_dino+siglip+nomrc+2+gelu2-test/ckpts/best_val_unseen \
