...
name=${name}-seed.${seed}
name=${name}-init.aug.45k-new_dino+siglip+nomrc+1+gelu2+va1
...
CUDA_VISIBLE_DEVICES="1" python3 -m torch.distributed.launch --node_rank 0 --nnodes=1  --master_port 29525 --nproc_per_node=${ngpus}  map_nav_src_fullmodule/main_nav.py $flag \
  --tokenizer bert --test --submit\
  --resume_file  datasets/R2R/exprs_map/finetune/Grid_Map-dagger-dino_imagenet-seed.0-init.aug.45k-new_dino+siglip+nomrc+1+gelu2/ckpts/best_val_unseen \
