...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        if not token_type_ids.dtype == torch.long:
...
class SpatialSemanticFusion(nn.Module):
    def __init__(self):
        super(SpatialSemanticFusion, self).__init__()

    def forward(self, grid_fts_b: torch.Tensor, spatial_text_b: torch.Tensor):
        """
        grid_fts_b: Tensor of shape [n*12*64, 768]
        spatial_text_b: Tensor of shape [n, 12, 768]
        """
        n = spatial_text_b.shape[0]  # 导航点数
        assert grid_fts_b.shape[0] == n * 12 * 64, "grid_fts_b shape mismatch with spatial_text_b"

        # 变回 [n, 12, 64, 768]
        grid_fts_b = grid_fts_b.view(n, 12, 64, 768)

        fused_grid_fts_b = []

        for i in range(n):  # 遍历每个导航点
            patch_fts = grid_fts_b[i]       # [12, 64, 768]
            spatial_fts = spatial_text_b[i] # [12, 768]

            fused_views = []
            for j in range(12):  # 每个视角方向
                direction_patches = patch_fts[j]   # [64, 768]
                spatial_vec = spatial_fts[j]       # [768]

                # attention score: [64]
                attn_scores = direction_patches @ spatial_vec
                attn_weights = F.softmax(attn_scores, dim=0)  # [64]

                # 加权求和
                fused = direction_patches * attn_weights.unsqueeze(-1)  # [64, 768]
                fused_views.append(fused)

            # 拼接12个方向：[12*64, 768]
            fused_nav = torch.cat(fused_views, dim=0)
            fused_grid_fts_b.append(fused_nav)

        # 拼接所有导航点：[n*12*64, 768]
        fused_grid_fts_b = torch.cat(fused_grid_fts_b, dim=0)

        return fused_grid_fts_b
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.fusion_module = SpatialSemanticFusion()
    ...
    def forward_text(self, txt_ids, txt_masks):
        ...
        if not txt_ids.dtype == torch.long:
            txt_ids = txt_ids.long()
    ...
    def forward_panorama_per_step(
        ...
        if has_text:
            # 处理文本特征
            view_text_embeds = self.img_embeddings.text_layer_norm(
                self.img_embeddings.text_linear(view_img_text_fts)
            )
        ...
        if has_obj:
            if self.img_embeddings.obj_linear is None:
                ...
                #print("img_embeds.shape:", obj_img_embeds.shape)
    ...
    def forward_navigation_per_step(
            ...
            vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids, grid_spatial_text,grid_fts, grid_map,
            gridmap_pos_fts
    ):
        ...
        for b in range(batch_size):
            fused_grid_fts_b = self.fusion_module(grid_fts[b].float(), grid_spatial_text[b].float())
            tmp_fts = grid_fts[b].float()
            # print(f"tmp_fts dtype: {tmp_fts.dtype}, text_fts[b] dtype: {text_fts[b].dtype}")
            grid_fts_weight, _ = (fused_grid_fts_b@ text_fts[b]).max(dim=-1)
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            pano_embeds, pano_masks = self.forward_panorama_per_step(
                batch['view_img_fts'], batch['view_text_fts'],  batch['obj_img_fts'], batch['loc_fts'],
            ...
            return self.forward_navigation_per_step(
                ...
                batch['vp_nav_masks'], batch['vp_obj_masks'], batch['vp_cand_vpids'], batch['grid_spatial_text'], batch['grid_fts'],
