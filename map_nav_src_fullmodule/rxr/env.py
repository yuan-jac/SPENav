...
class EnvBatch(object):
    ...
    def getGlobalMap(self, tid):
        ...
        if len(self.global_semantic[i]) == 0:
            ...
            self.spatial_text_fts[i] = [SpatialText]  # list of [12, 768]
            self.global_map[i] = np.zeros((12 * 64,))

        else:
            if not isinstance(self.spatial_text_fts[i], list):
                self.spatial_text_fts[i] = list(self.spatial_text_fts[i])
            self.spatial_text_fts[i].append(SpatialText)
        ...
        self.spatial_text_fts[i]=np.stack(self.spatial_text_fts[i],axis=0)

        return tid,  self.spatial_text_fts[i],self.global_semantic[i], self.global_position_x[i], self.global_position_y[i], self.global_mask[i], \
    ...
    def getStates(self):
        ...
        for i in range(len(self.sims)):
            ...
            tid,self.spatial_text_fts[i], self.global_semantic[i], self.global_position_x[i], self.global_position_y[i], self.global_mask[i], \
