...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        '''for i in range(len(obs)):
            print(f"[DEBUG] obs[{i}]['spatial_fts'] type: {type(obs[i]['spatial_fts'])}")
            print(f"[DEBUG] obs[{i}]['spatial_fts'] len: {len(obs[i]['spatial_fts'])}")
            print(f"[DEBUG] obs[{i}]['spatial_fts'][0] type: {type(obs[i]['spatial_fts'][0])}")
            print(f"[DEBUG] obs[{i}]['spatial_fts'][0] shape or len: "
                  f"{obs[i]['spatial_fts'][0].shape if isinstance(obs[i]['spatial_fts'][0], torch.Tensor) else len(obs[i]['spatial_fts'][0])}")'''
        return {
