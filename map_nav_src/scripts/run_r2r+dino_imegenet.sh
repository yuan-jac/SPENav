...
name=${name}-seed.${seed}
name=${name}-init.aug.45k-new_dino+siglip+mrc+gate_text+gelu+reset-1module1
...
CUDA_VISIBLE_DEVICES='4' python3 -m torch.distributed.launch --node_rank 0 --nnodes=1  --master_port 29511 --nproc_per_node=${ngpus}  map_nav_src/main_nav.py $flag \
    --tokenizer bert \
    --bert_ckpt_file datasets/R2R/exprs_map/pretrain/cmt-vitbase-mlm.mrc.sap-init.lxmert-aug.speaker-new-nocp-2-gate-text/ckpts/model_step_97500.pt \

    #--resume_file  datasets/R2R/exprs_map/finetune/Grid_Map-dagger-dino_imagenet-seed.0-init.aug.45k-new_dino+siglip+nomrc/ckpts/best_val_unseen \
  #--eval_first
