...
name=${name}-seed.${seed}
name=${name}-init.aug.45k-new_dino+siglip+nomrc+gate_text+gelu
...
CUDA_VISIBLE_DEVICES='0,1,4' python3 -m torch.distributed.launch --node_rank 0 --nnodes=1  --master_port 29511 --nproc_per_node=${ngpus}  map_nav_src/main_nav.py $flag \
    --tokenizer bert \
